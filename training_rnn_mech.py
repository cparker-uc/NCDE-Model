...
from testing_rnn_mech import main as test
...
def main():
    ...
    # domain = torch.tensor([0,15,30,40,50,65,80,95,110,125,140], dtype=torch.double, requires_grad=True)
    ...
    for (data, _) in loader:
        ...
        res = differential_evolution(de_loss, bounds, args=(data,y0), disp=True, maxiter=500)
        sol = func(res.x, y0)
    ...
    for j, (data, labels) in enumerate(loader):
        ...
        with torch.no_grad():
            test(individual_number=INDIVIDUAL_NUMBER)
...
def loss(pred_y: torch.Tensor, labels: torch.Tensor,
    ...
    crh_loss = l1_loss(pred_y[:,0], mech_solution[:,1])
    ...
    data_loss = l1_loss(pred_y_datapoints[...,[1,2]], labels[...,1:])
...
def save_network(model: RNN, optimizer: optim.AdamW, itr: int, runtime: float, loss_over_time: list[float]):
